...
BOLD.helpers.setPriceWithOptions = function(cart, base_item) {
    ...
    var total_price = base_item.price,
        one_time_charges = 0,
        builder_id = base_item.Bold.builder_id;
...
BOLD.helpers.removeBuilder = function(builder_id, success_callback, error_callback) {
    if (typeof(builder_id) !== 'undefined')
        BOLD.helpers.getCart(function(cart) {
            BOLD.helpers.removeItemWithOptions(cart, builder_id, success_callback, error_callback)
        });
}
...
BOLD.helpers.upateBuilder = function(builder_id, qty, success_callback, error_callback) {
    if (typeof(builder_id) !== 'undefined')
        BOLD.helpers.getCart(function(cart) {
            BOLD.helpers.updateItemWithOptions(cart, builder_id, qty, success_callback, error_callback)
        });
}
...
BOLD.helpers.setCartAttrObject = function(name, attributes, success_callback, error_callback) {
    ...
    var params = {
        ...
        dataType: 'json',
        data: data,
        success: success_callback || function(c) {
            console.info(c)
        },
        error: error_callback || function(a, b) {
            console.error(a, b)
        }
    };
    jQuery.ajax(params);
...
BOLD.helpers.getCartAttrObj = function(name, cart) {
    if (typeof(cart) !== 'object') {
        BOLD.helpers.getCart(function(c) {
            BOLD.helpers.getCartAttrObj(name, c)
        });
        return;
    }
    try {
...
BOLD.helpers.calcOptionsTotal = function(form) {
    ...
    var totalExtras = 0;
    extrasList.each(function(index, obj) {
        totalExtras += jQuery(obj).data('price') * 100;
    });
    return parseInt(totalExtras);
};
BOLD.helpers.calcTotal = function(variant, form) {
    return BOLD.helpers.calcBTMTotal(variant, form) + BOLD.helpers.calcOptionsTotal(form);
};
...
BOLD.helpers.rivetsUpdate = function() {
    CartJS.updateItem = function(line, qty) {
        BOLD.helpers.changeItem(line, qty, function(cart) {
